...
while [ $actualpoint -le $numpoints ]; do
	...
	if [ "$actualclient" = "eixam" ]; then
		rm -f points-eixam-sm
		echo "no" > eixam_sm_finished
		j=1
		startindex=$actualpoint
		while [ $j -le $nprocess ];do
			tail -n +$actualpoint $pointsfile | head -n 1 >> points-eixam-sm
			let j=$j+1
			let actualpoint=$actualpoint+1
		done
		(
		rsync points-eixam-sm agranados@eixam.upc.es:/home/agranados/scattering_map/
		ssh agranados@eixam.upc.es "cd scattering_map && ./distribute_sm2.sh points-eixam-sm $numiterates $delta $startindex > process_distribute_sm2"
		)&
	else
		...
		clientarch=$a$b
		clientsystem=$clientarch/find_sm
